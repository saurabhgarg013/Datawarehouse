...
--he events with event_date = '2023-12-14' are considered today's data 
...
CREATE TABLE tech.active_users_daily (
    user_id INTEGER,
...
---To calculate daily active users, we group by user_id and calculate metrics 
---like num_likes, num_comments, and num_shares for a specific day.
...
SELECT
    ...
    CAST('2023-12-14' AS DATE) AS snapshot_date
FROM events
WHERE event_date = '2023-12-14'
GROUP BY user_id;

--concept of unrest--

SELECT COUNT(*)
FROM unnest('{1, -2, 3, 4, 5, -6, 7}'::int[]) AS share_value
WHERE share_value > 0;
...
--delete from tech.active_users_daily;
select user_id,event_date  from tech.events order by event_date;
-- start with date 2023-12-10,then 2023-12-11  ,then 2023-12-12
...
INSERT INTO tech.active_users_daily
SELECT
    user_id,
    -- If the user_id has at least 1 event, they are daily active
    CASE WHEN COUNT(user_id) > 0 THEN 1 ELSE 0 END AS is_active_today,
    ...
    COUNT(CASE WHEN event_type = 'comment' THEN 1 END) AS num_comments,
    COUNT(CASE WHEN event_type = 'share' THEN 1 END) AS num_shares,
    CAST('2023-12-10' AS DATE) AS snapshot_date
FROM tech.events
WHERE event_date = '2023-12-10'
GROUP BY user_id;


select * from tech.active_users_daily where snapshot_date = '2023-12-10' ;
...
SELECT
    ...
    COUNT(CASE WHEN event_type = 'comment' THEN 1 END) AS num_comments,
    COUNT(CASE WHEN event_type = 'share' THEN 1 END) AS num_shares,
    CAST('2023-12-11' AS DATE) AS snapshot_date
FROM tech.events
WHERE event_date = '2023-12-11'
GROUP BY user_id;


select * from tech.active_users_daily where snapshot_date = '2023-12-11' ;
...
select * from tech.active_users_daily;
...
SELECT
    ...
    COUNT(CASE WHEN event_type = 'comment' THEN 1 END) AS num_comments,
    COUNT(CASE WHEN event_type = 'share' THEN 1 END) AS num_shares,
    CAST('2023-12-13' AS DATE) AS snapshot_date
FROM tech.events
WHERE event_date = '2023-12-13'
GROUP BY user_id;
...
CREATE TABLE tech.active_users_cumulated (
    user_id INTEGER,
...
-- select * from tech.active_users_cumulated order by snapshot_date ;
-- startwith date yesterday '2023-12-09' and today '2023-12-10' then '2023-12-10' and '2023-12-11'
-- and then '2023-12-11' and '2023-12-12' and '2023-12-12' and '2023-12-13' and '2023-12-13' and '2023-12-14'
-- make sure data also insert daily in tech.active_users_daily first then insert tech.active_users_cumulated;
select * from tech.active_users_cumulated order by snapshot_date


INSERT INTO tech.active_users_cumulated

-- First read in yesterday's data from the cumulated table
WITH yesterday AS (
    SELECT * 
    FROM tech.active_users_cumulated
    WHERE snapshot_date = '2023-12-12'
),
-- Read in today's active user data from the daily table
today AS (
    SELECT * 
    FROM tech.active_users_daily
    WHERE snapshot_date = '2023-12-13'
),
...
combined AS (
    SELECT
       ...
       COALESCE(t.snapshot_date,'2023-12-14') as snapshot_date   -- 2023-12-14'  this date when todays run data today snap shot date come run
